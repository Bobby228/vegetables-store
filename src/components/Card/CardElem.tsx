import {Title, Card, Image, Text, Button, Group} from "@mantine/core";
...
const CardElem = (
  {id, name, price, image}: ProductProps) => {
    ...
    <>
      <Card w={302} p={16}>
        ...
        <Group justify="space-between" mt={16}>
          <Title order={4}>{name.split('-')[0]} <span className={styles.card__span}>{name.split('-')[1]}</span></Title>
          <Group gap={9}>
            <Button w={30} h={30} p={0} c='black' color="rgba(222, 226, 230, 1)" style={{ fontSize: 20 }} onClick={() => setLocalCount(c => Math.max(0, c - 1))}>
              -
            </Button>
            <span>{localCount}</span>
            <Button w={30} h={30} p={0} c='black' color="rgba(222, 226, 230, 1)" style={{ fontSize: 20 }} onClick={() => setLocalCount(c => c + 1)}>
              +
            </Button>
          </Group>
        </Group>
